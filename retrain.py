...
from nmprun import load_pruning_masks
...
# 加载剪枝后获得的mask
mask_path = 'models/swin_pruning_masks.pt'
...
def retrain_pruned_model():
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if test_acc > best_acc:
            best_acc = test_acc
            torch.save(model.state_dict(), f'models/swin_cifar10_pruned_best.pth')
            print(f'Saved best pruned model with accuracy: {best_acc:.3f}%')

    print(f'Retraining complete. Best test accuracy: {best_acc:.3f}%')
    return best_acc
